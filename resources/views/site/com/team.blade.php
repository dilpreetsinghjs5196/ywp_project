...
@section('content')
  ...
  <section class="section py-5">
    <div class="b-container">
        ...
        <div class="col-12">
          <div class="card border-0 shadow-sm rounded-4 p-4 bg-white">
            <div class="row g-3 align-items-center">
              <div class="col-md-4">
                <label class="form-label fw-bold small text-muted mb-1"><i class="bi bi-heart me-1"></i> Specialization / Concern</label>
                <select id="concern-filter" class="form-select rounded-pill border-2">
              ...
              <div class="col-md-3">
                <label class="form-label fw-bold small text-muted mb-1"><i class="bi bi-translate me-1"></i> Language</label>
                <select id="language-filter" class="form-select rounded-pill border-2">
              ...
              <div class="col-md-3">
                <label class="form-label fw-bold small text-muted mb-1"><i class="bi bi-geo-alt me-1"></i> Session Mode</label>
                <select id="mode-filter" class="form-select rounded-pill border-2">
              ...
              <div class="col-md-2 d-flex align-items-end h-100">
                <button id="reset-filters" class="btn btn-outline-secondary rounded-pill w-100 mt-4">
      ...
      <!-- Team Members Grid -->
      <div class="row g-4" id="team-grid">
        @include('site.com.partials._team_list')
      </div>
...
@push('js')
  <script>
    $(document).ready(function() {
      ...
      const filters = $('#concern-filter, #language-filter, #mode-filter');

      function fetchFilteredTeams() {
        const concern = $('#concern-filter').val();
        const language = $('#language-filter').val();
        const mode = $('#mode-filter').val();

        teamGrid.css('opacity', '0.5');
        ...
        $.ajax({
          ...
          error: function() {
            teamGrid.css('opacity', '1');
            console.error('Failed to fetch filtered therapists');
          }
        });
      }
