            ...
            @forelse($appointments as $appointment)
                <tr>
                    ...
                    <td>
                        ...
                        @endphp
                        <span class="badge {{ $statusClass }} px-3 text-capitalize">{{ $appointment->status }}</span>
                    </td>
                    <td class="text-end pe-4">
                        <div class="btn-group">
                            ...
                            <form action="{{ route('admin.appointments.destroy', $appointment->id) }}" method="POST"
                                class="d-inline" onsubmit="return confirm('Are you sure you want to delete this inquiry?')">
                                @csrf
                                @method('DELETE')
    ...
    <div class="card-footer bg-white border-top">
        <div class="pagination-wrapper">
            <div class="text-muted small">
                Showing {{ $appointments->firstItem() }} to {{ $appointments->lastItem() }} of {{ $appointments->total() }}
                results
            </div>
