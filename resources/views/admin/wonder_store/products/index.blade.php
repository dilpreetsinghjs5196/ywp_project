...
@section('content')
    <div class="card">
        <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
            ...
            <a href="{{ route('admin.wonder-store-products.create') }}" class="btn btn-primary btn-sm">
                <i class="bi bi-plus-lg"></i> Add New Product
            </a>
        </div>
        <div class="card-body bg-light border-bottom p-3">
            <form id="filterForm" action="{{ route('admin.wonder-store-products.index') }}" method="GET" class="row g-2">
                <div class="col-md-5">
                    <div class="input-group input-group-sm">
                        <span class="input-group-text bg-white border-end-0"><i class="bi bi-search"></i></span>
                        <input type="text" name="search" id="searchInput" class="form-control border-start-0"
                            placeholder="Search by Product Description or Category..." value="{{ request('search') }}">
                    </div>
                </div>
                <div class="col-md-2">
                    <button type="submit" class="btn btn-primary btn-sm px-4">Filter</button>
                    <a href="{{ route('admin.wonder-store-products.index') }}" id="clearBtn"
        ...
        <div class="card-body p-0" id="tableContainer">
            @include('admin.wonder_store.products._table')
        </div>
    </div>
@endsection
