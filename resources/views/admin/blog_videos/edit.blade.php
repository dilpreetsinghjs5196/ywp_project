        ...
        <div class="col-md-8">
            <div class="card border-0 shadow-sm">
                <div class="card-header bg-white py-3">
                    <h5 class="mb-0 fw-bold">Edit Video</h5>
                </div>
                <div class="card-body p-4">
                    <form action="{{ route('admin.blog-videos.update', $video->id) }}" method="POST"
                        enctype="multipart/form-data">
                        @csrf
                        @method('PUT')

                        <div class="mb-3">
                            <label for="video_theme_id" class="form-label">Video Theme <span
                                    class="text-danger">*</span></label>
                            <select name="video_theme_id" id="video_theme_id"
                                class="form-select @error('video_theme_id') is-invalid @enderror" required>
                                <option value="">Select Video Theme</option>
                                @foreach($themes as $theme)
                                    <option value="{{ $theme->id }}" {{ old('video_theme_id', $video->video_theme_id) == $theme->id ? 'selected' : '' }}>
                                        {{ $theme->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            @if($video->thumbnail)
                                <div class="mt-2">
                                    <img src="{{ asset('storage/' . $video->thumbnail) }}" alt="Preview" class="img-thumbnail"
                                        style="height: 100px;">
                                </div>
                            @endif
                            @error('thumbnail')
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <button type="submit" class="btn btn-primary px-5">Update Video</button>
                        </div>
                    </form>
                </div>
            </div>
